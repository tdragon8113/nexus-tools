...
import { hasNoteContent } from '../noteRichText';
...
export default function HomePage({
    ...
    const todayActivities = getTodayActivities(activities, categories);
    const todayDuration = getTodayDuration(activities, categories);
    ...
    const todayEntryCount = todayActivities.length + (activeRecordingToday ? 1 : 0);

    const todayCategoryStats = useMemo(() => {
        const stats = getTodayCategoryStats(activities, categories);
    ...
    const timelineEntries = useMemo((): TimelineEntry[] => {
        const savedEntries: TimelineEntry[] = todayActivities.map((activity) => ({
            kind: 'saved',
            activity,
        }));
    ...
    }, [todayActivities, activeRecordingToday]);
    ...
    const dayTimeline = useMemo(() => {
        ...
        const timelineItems = timelineEntries.map((entry) => {
            if (entry.kind === 'recording') {
                const startMs = new Date(entry.session.startedAt).getTime();
            ...
            const startMs = new Date(getActivityStartAt(entry.activity)).getTime();
            const endMs = new Date(entry.activity.createdAt).getTime();
            ...
            return {
                ...
                endMs,
                isLive: false,
                title: `${meta.label} ${formatTimeLabel(getActivityStartAt(entry.activity))}–${formatTimeLabel(entry.activity.createdAt)}`,
            };
        });
