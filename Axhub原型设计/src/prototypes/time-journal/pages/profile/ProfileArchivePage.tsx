...
import {
    ...
    formatTimeLabel,
    formatYearKey,
    getActivityAttributionDateKey,
    getActivitiesSortedDesc,
    getActivityStartAt,
    getCategoryMeta,
    getRecordDailyTracks,
...
type ProfileArchivePageProps = {
    ...
    onUpsertReflection?: (scope: ReflectionScope, period: string, content: string) => void;
};
...
export default function ProfileArchivePage({
    ...
    const duplicatePeriod =
        editorMode === 'new'
        && sortedSummaries.some((item) => item.date === draftPeriod);
    ...
    const saveSummary = () => {
        if (!onUpsertReflection || !summaryConfig || !hasNoteContent(draftContent) || !draftPeriod) {
            return;
        }
        onUpsertReflection(summaryConfig.scope, draftPeriod, draftContent);
        closeEditor();
    };
        ...
        <div className={subpageClassName}>
            <header className="tj-subpage-sticky-head">
                ...
                {kind === 'record-days' ? (
                    ...
                    <div className="tj-subpage-title-row">
                        <h1>{copy.title}</h1>
                        {!isSummaryEditing && onUpsertReflection ? (
                                ...
                                editorMode === item.id ? (
                                    ...
                                    >
                                        <div className="tj-archive-reflection-head">
                                            ...
                                            {onUpsertReflection && editorMode === 'closed' ? (
                                                <button
                                                    type="button"
