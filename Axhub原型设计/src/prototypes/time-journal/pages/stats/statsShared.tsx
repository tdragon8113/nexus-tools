...
import {
    ...
    filterActivitiesForStats,
    formatDuration,
    formatStatsRangeLabel,
    getActivitiesInDateRange,
    getCategoryMeta,
    getCategoryTimelineColor,
    getPeriodCategoryBreakdown,
    getPeriodChartBuckets,
    getPeriodDayMarkers,
    getPeriodMetrics,
    getStatsRangeBounds,
    getStreakDays,
...
export type StatsViewProps = {
    ...
    dayMarkers: ReturnType<typeof getPeriodDayMarkers>;
    rangeLabel: string;
};

export function useStatsViewProps(
    activities: Activity[],
    categories: ActivityCategoryConfig[],
    rangeSelection: StatsRangeSelection,
    excludeSleep: boolean,
): Omit<StatsViewProps, 'setRangeSelection' | 'setExcludeSleep'> {
    ...
    const bounds = useMemo(
        () => getStatsRangeBounds(rangeSelection),
        [rangeSelection],
    );
    const metrics = useMemo(
        ...
        [filteredActivities, categories, bounds],
    );
    ...
    const chartBuckets = useMemo(
        () => getPeriodChartBuckets(filteredActivities, categories, bounds),
        [filteredActivities, categories, bounds],
    );
    const streak = useMemo(
        () => getStreakDays(filteredActivities, categories),
    ...
    const insight = useMemo(
        ...
        [bounds, metrics, previousMetrics, categoryBreakdown, chartBuckets, categories],
    );
    ...
    const totalChange = calcStatsChangePercent(metrics.totalMinutes, previousMetrics.totalMinutes);
    const avgChange = calcStatsChangePercent(
        metrics.avgDailyMinutes,
        previousMetrics.avgDailyMinutes,
    );
    const xpChange = calcStatsChangePercent(metrics.totalXp, previousMetrics.totalXp);
    const maxBucketMinutes = Math.max(...chartBuckets.map((item) => item.minutes), 1);
    const maxCategoryMinutes = Math.max(...categoryBreakdown.map((item) => item.minutes), 1);
    const moodLabel =
        metrics.avgMood !== null
            ? moodLabels[Math.min(moodLabels.length, Math.max(1, Math.round(metrics.avgMood))) - 1]
            : null;
    ...
    return {
        ...
        moodLabel,
        dayMarkers,
        rangeLabel: formatStatsRangeLabel(bounds.startKey, bounds.endKey),
    };
}
