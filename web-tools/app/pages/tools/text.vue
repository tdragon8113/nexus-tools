      ...
      <div class="flex flex-wrap items-center gap-2">
        ...
        <div class="flex flex-wrap items-center gap-1.5 text-xs text-slate-600">
          ...
          <button
            ...
            :disabled="formatting || !hasEditorContent || !canFormat"
            :title="canFormat ? formatTip : '当前语法不支持格式化'"
            :aria-label="canFormat ? formatTip : '当前语法不支持格式化'"
            @click="formatDocument({ toastOnSuccess: true })"
          >
            {{ formatting ? '格式化中…' : '格式化' }}
          </button>
        </div>
...
const downloadTip = computed(() => {
  if (language.value === 'markdown') return '下载为 .md'
  if (language.value === 'csv') return '下载为 .csv'
  return '下载为 .txt'
})
...
watch(language, async (id, prev) => {
  ...
  if (!languageReady.value || !prev || prev === id) return
  if (!canFormatTextDiffLanguage(id)) return

  const switchingToJson = id === 'json' && prev !== 'json'
  const switchingToCsv = id === 'csv' && prev !== 'csv'
  const showFormatFeedback = switchingToJson || switchingToCsv
  await formatDocument({
    silentOnError: !showFormatFeedback,
    toastOnSuccess: showFormatFeedback
  })
})
...
async function formatDocument(options?: { toastOnSuccess?: boolean; silentOnError?: boolean }) {
  if (formatting.value) return
  if (!canFormat.value) {
    if (!options?.silentOnError) {
      showToast('当前语法不支持格式化')
    }
    return
  }
  ...
  try {
    ...
    if (formatted === source) {
      if (options?.toastOnSuccess) showToast('已是规范格式')
    } else {
      applyText(formatted, options?.toastOnSuccess ? formatActionLabel(language.value) : undefined)
    }
  } catch (e) {
    if (!options?.silentOnError) {
...
function onFileSelected(e: Event) {
  ...
  reader.onload = () => {
    applyText(String(reader.result ?? ''), `已导入 ${file.name}`)
    if (lowerName.endsWith('.csv')) {
      void formatDocument({ silentOnError: true })
    }
  }
...
function downloadText() {
  ...
  const isCsv = language.value === 'csv'
  ...
  a.href = url
  a.download = isMd ? 'document.md' : isCsv ? 'document.csv' : 'document.txt'
  a.click()
